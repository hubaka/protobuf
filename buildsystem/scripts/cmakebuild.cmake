...
macro(_config_module)
	...
	if(NOT BFW_ROOT_DIR)
		...
		set(${PROJECT_ID}_PROTO_SRC)				# Macro to store .proto files
		set(${PROJECT_ID}_PROTO_HDR_FILES)				# Macro to store generated headers of proto
...
macro(_add_header_file hdr_file)
	...
	if( ${hdr_file_absolute_path} MATCHES \\.pb.h$|\\.pb.hpp$ )
		# This file will be generated by the proto compiler, it doesn't exist at this moment
		list(APPEND ${PROJECT_NAME}_PROTO_HDR_FILES ${hdr_file_absolute_path})
	elseif (${hdr_file_absolute_path} MATCHES \\.h$|\\.hpp$)
...
macro(_add_source_file src_file)
	...
	if (${src_file_absolute_path} MATCHES \\.proto$)
		list(APPEND ${PROJECT_ID}_PROTO_SRC ${src_file_absolute_path})
...
macro(install_module_lib)
	...
	if(${PROJECT_ID}_PROTO_HDR_FILES)
		list(REMOVE_DUPLICATES ${PROJECT_ID}_PROTO_HDR_FILES)
	endif()
	...
	if(_install_hdrs)
		...
		foreach(_header ${${PROJECT_ID}_PROTO_HDR_FILES})
			get_filename_component(_name ${_header} NAME)
			list(APPEND _hdrs ${_name})
		endforeach()
...
macro(_add_module_lib)
	if (DEBUG_PRINTS)
		...
		if (${PROJECT_ID}_PROTO_SRC)
			_add_proto_src(${${PROJECT_ID}_PROTO_SRC})
		endif()
		message(STATUS "aka: src_lib ${${PROJECT_ID}_SRC}")
...
macro (_add_proto_src)
	if (TRUE)
		message(STATUS "aka: _add_proto_src ${PROJECT_ID}")
	endif()
	...
	include_directories(${proto_install_directory})	
endmacro()
